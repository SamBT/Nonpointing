...
int main (int argc, char **argv) {
  ...
  vector<double> zdca_bins = {0,20,40,60,80,100,120,140,160,180,200,2000};
  ...
  TH1F *h_leading_timing = new TH1F("h_leading_timing","h_leading_timing",400,-20,20);
  TH1F *h_leading_pointing = new TH1F("h_leading_pointing","h_leading_pointing",11,0,11);
  ...
  TH1F *h_subleading_timing = new TH1F("h_subleading_timing","h_subleading_timing",400,-20,20);
  TH1F *h_subleading_pointing = new TH1F("h_subleading_pointing","h_subleading_pointing",11,0,11);
  ...
  for( int i = 0 ; i < numev ; i ++ ) {	
    ...
    if (passObj1 == true && passObj2 == false) {
      ...
      for (int i = 0; i < zdca_bins.size()-1; i++) {
	if (fabs(pt1) > zdca_bins[i] && fabs(pt1) < zdca_bins[i+1]) {
	  zdca_bin = i+1;
	}
      }
      h_leading_pointing->Fill(zdca_bin,wt);
    }
      ...
      for (int i = 0; i < zdca_bins.size()-1; i++) {
	if (fabs(pt2) > zdca_bins[i] && fabs(pt2) <zdca_bins[i+1]){
          zdca_bin = i+1;
        }
      } 
      h_leading_pointing->Fill(zdca_bin,wt);
      ...
      if (leading == 1) {
	for (int i = 0; i < zdca_bins.size()-1; i++) {
	  if (fabs(pt1) > zdca_bins[i] && fabs(pt1) <zdca_bins[i+1]) zdca_bin = i+1;
	  if (fabs(pt2) > zdca_bins[i] && fabs(pt2) < zdca_bins[i+1]) zdca_bin_sub = i+1;
	} 
	h_leading_pt->Fill(p->tru_ph1_pt,wt);
	h_leading_eta->Fill(p->tru_ph1_eta,wt);
	h_leading_timing->Fill(p->tru_ph1_timing,wt);
	...
	h_subleading_pointing->Fill(zdca_bin_sub,wt);
      }
      if (leading == 2) {
	for (int i = 0; i < zdca_bins.size()-1; i++) {
          if (fabs(pt1) > zdca_bins[i] && fabs(pt1) <zdca_bins[i+1]) zdca_bin_sub =i+1;
          if (fabs(pt2) > zdca_bins[i] && fabs(pt2) < zdca_bins[i+1]) zdca_bin = i+1;
        }
	h_leading_pt->Fill(p->tru_ph2_pt,wt);
	h_leading_eta->Fill(p->tru_ph2_eta,wt);
	h_leading_timing->Fill(p->tru_ph2_timing,wt);
    ...
    h_CF->Fill(1.5,wt); //pt > 50 for both
    h_CF_unwt->Fill(1.5);
    if (fabs(eta1) > 1.37 && fabs(eta2) > 1.37) continue;
    h_CF->Fill(2.5,wt); //at least one photon in barrel
    h_CF_unwt->Fill(2.5);
    ...
    if (met > 75 && timing > 1.5) {h_CF->Fill(5.5,wt); h_CF_unwt->Fill(5.5); cout << "met > 75, t > 1.5, timing = " << timing << ", event num = " << i << endl;}
    if (met > 75 && timing > 2) {h_CF->Fill(6.5,wt);h_CF_unwt->Fill(6.5); cout << "met > 75, t > 2, timing = " << timing << ", event num = " << i << endl;}
    if (met > 75 && timing > 2 && fabs(zdca) > 200) {h_CF->Fill(7.5,wt);h_CF_unwt->Fill(7.5);}
    if (met > 75 && timing < 4) {h_CF->Fill(8.5,wt); h_CF_unwt->Fill(8.5);}
