...
int main (int argc, char **argv) {
	...
	TEfficiency *h_pt2_trigger_eff_pt1_40 = new TEfficiency("h_pt2_trigger_eff_pt1_40","h_pt2_trigger_eff_pt1_40",2000,0,2000);

	TH2F *h_Ecell_gain = new TH2F("h_Ecell_gain","h_Ecell_gain",1000,0,1000,3,0,3);
	TH1F *h_Ecell_lowgain = new TH1F("h_Ecell_lowgain","h_Ecell_lowgain",1000,0,1000);
	TH1F *h_Ecell_medgain = new TH1F("h_Ecell_medgain","h_Ecell_medgain",1000,0,1000);
	TH1F *h_Ecell_highgain = new TH1F("h_Ecell_highgain","h_Ecell_highgain",1000,0,1000);
	TH1F *h_Ecell_allgains = new TH1F("h_Ecell_allgains","h_Ecell_allgains",1000,0,1000);
	TEfficiency *h_Ecell_lowgain_veto_eff = new TEfficiency("h_Ecell_lowgain_eff","h_Ecell_lowgain_eff",1000,0,1000);
	...
	for( int i = 0 ; i < numev ; i ++ ) {	
	    ...
	    if( p->HLT_g35_loose_g25_loose == 1  && p->ph_pt2 > 50 ) h_CF -> Fill( 1.5, wt);
	    if( p->HLT_g35_loose_g25_loose == 1  && p->ph_pt2 > 50 && p->ph_passIso1 && p->ph_passIso2 ) h_CF -> Fill( 2.5, wt);
	    if( p->HLT_g35_loose_g25_loose == 1  && p->ph_pt2 > 50 && p->ph_passIso1 && p->ph_passIso2 && p->m_met > 75 ) h_CF -> Fill( 3.5, wt);
	    if( p->HLT_g35_loose_g25_loose == 1  && p->ph_pt2 > 50 && p->ph_passIso1 && p->ph_passIso2 && p->m_met > 75 && (fabs(p->ph_eta1) < 1.37 || fabs(p->ph_eta2) < 1.37)) h_CF -> Fill( 4.5, wt);
	    if( p->HLT_g35_loose_g25_loose == 1  && p->ph_pt2 > 50 &&                                     p->m_met < 20 && (fabs(p->ph_eta1) < 1.37 || fabs(p->ph_eta2) < 1.37)) h_CF -> Fill( 5.5, wt);
	    
	    
	    bool lowgain, medgain, highgain;
	    if (p->ph_pt1 > 50 && fabs(p->ph_eta1) < 1.37) {
	      lowgain = false; medgain = false; highgain = false;
	      h_Ecell_gain->Fill(p->ph1_maxEcell_E/1000,p->ph1_maxEcell_gain);
	      if (p->ph1_maxEcell_gain == 2) lowgain = true;
	      if (p->ph1_maxEcell_gain == 1) medgain = true;
	      if (p->ph1_maxEcell_gain == 0) highgain = true;
	      h_Ecell_allgains->Fill(p->ph1_maxEcell_E/1000);
	      if (lowgain) h_Ecell_lowgain->Fill(p->ph1_maxEcell_E/1000);
	      if (medgain) h_Ecell_medgain->Fill(p->ph1_maxEcell_E/1000);
	      if (highgain) h_Ecell_highgain->Fill(p->ph1_maxEcell_E/1000);
	      h_Ecell_lowgain_veto_eff->Fill(!lowgain,p->ph1_maxEcell_E/1000);
	    }
	    if (p->ph_pt2 > 50 && fabs(p->ph_eta2) < 1.37) {
              lowgain = false; medgain = false; highgain = false;
              h_Ecell_gain->Fill(p->ph2_maxEcell_E/1000,p->ph2_maxEcell_gain);
              if (p->ph2_maxEcell_gain == 2) lowgain = true;
              if (p->ph2_maxEcell_gain == 1) medgain = true;
              if (p->ph2_maxEcell_gain == 0) highgain = true;
              h_Ecell_allgains->Fill(p->ph2_maxEcell_E/1000);
              if (lowgain) h_Ecell_lowgain->Fill(p->ph2_maxEcell_E/1000);
              if (medgain) h_Ecell_medgain->Fill(p->ph2_maxEcell_E/1000);
              if (highgain) h_Ecell_highgain->Fill(p->ph2_maxEcell_E/1000);
              h_Ecell_lowgain_veto_eff->Fill(!lowgain,p->ph2_maxEcell_E/1000);
            }
		...
		if (p->ph_passIso1 && p->ph_passIso2 && eta12) {
			for (int m = 1; m <= h_pt1_pass_trigger->GetNbinsX(); m++) {
				...
				if (p->ph_pt2 >= (m-1)*(2000/h_pt2_pass_trigger->GetNbinsX())) {
					...
					if (p->ph_pt1 > 40) h_pt2_trigger_eff_pt1_40->Fill(p->HLT_g35_loose_g25_loose,(m-1)*(2000/h_pt2_pass_trigger->GetNbinsX()));
				}
			}
		}
	...
	if( p->wt_xs != 0 ) {
	  ...
	  if( p-> m_met < 75 || !p->ph_passIso1 || !p->ph_passIso2 ) continue ;
	}
	else {
	...
	h_pt2_trigger_eff_pt1_40->Write();
	h_Ecell_gain->Write();
        h_Ecell_lowgain->Write();
        h_Ecell_medgain->Write();
        h_Ecell_highgain->Write();
        h_Ecell_allgains->Write();
        h_Ecell_lowgain_veto_eff->Write();
	f.Close();
