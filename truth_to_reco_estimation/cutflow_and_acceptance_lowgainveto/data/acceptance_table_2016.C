void acceptance_table_2016(int begin, int end) {
  ofstream t1;
  TString t1_name = "acceptance_data16_lowgainveto.tex";
  ...
  while (!info.eof()) {
    ...
    t1 << std::fixed << std::setprecision(0) << run_num << "&" << std::fixed << std::setprecision(2) << lumi << "&" << std::setprecision(0) << hs->GetBinContent(1) << "&" << std::setprecision(3) << hs->GetBinContent(3)/hs->GetBinContent(1) << "&" << hs->GetBinContent(4)/hs->GetBinContent(1) << "&" << hs->GetBinContent(5)/hs->GetBinContent(1) << "&" << hs->GetBinContent(14)/hs->GetBinContent(1) << "&" << std::setprecision(0) << hd->GetBinContent(1) << "&" << std::setprecision(3) << hd->GetBinContent(3)/hd->GetBinContent(1) << "&" << hd->GetBinContent(4)/hd->GetBinContent(1) << "&" << hd->GetBinContent(5)/hd->GetBinContent(1) << "&" << hd->GetBinContent(11)/hd->GetBinContent(1) << " \\\\ \n";
    t1 << "\\hline \n";
  }
 }
}
