void plot(TString rootfile, int run, int isdata) {
	...
	ofstream ltx;
	ltx.open("singlephoton.txt",std::ios_base::app);
	if (!isdata) {
...
void cutflow_table_latex() {
	...
	TString fname;
	TString data = "/global/homes/s/sambt/workarea/nonpointing/analysis_output/singlephoton/data/output_data.root";
	TString sig = "/global/homes/s/sambt/workarea/nonpointing/analysis_output/singlephoton/signal/";

	ofstream file1, file2;
	file1.open("singlephoton.txt");
	...
	file1.open("singlephoton.txt",std::ios_base::app);
	file1 << "\\end{tabular} \n";
	file1.close();
