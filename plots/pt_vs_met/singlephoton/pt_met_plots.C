...
void pt_met_plots() {
	...
	for (int i = 0; i < sig_runs.size(); i++) {
		tmp = sig_info[i];
		fname.Form("singlephoton_signal_pt_met_%i",sig_runs[i]);
		root.Form("/global/homes/s/sambt/workarea/nonpointing/analysis_output/singlephoton/signal/%i.root",sig_runs[i]);
