void plot_singlephoton_lowgainveto(TString variable, TString axis_label, int rebin, double xmin, double xmax, double ratio_min, double ratio_max) {
  ...
  TH2F *h15 = (TH2F*)f15->Get("h_"+variable+"_lowgainveto");
  h15->SetNameTitle("h15","h15");
  TH2F *h16 = (TH2F*)f16->Get("h_"+variable+"_lowgainveto");
  ...
  h16->Draw("histE");
  h15->Draw("histEsame");
  TString message = "Singlephoton, p_{T} > 150 GeV, Barrel";
  TLatex l;
  l.SetNDC();
  l.SetTextSize(0.05);
  l.DrawLatex(0.2,0.85,message);
  l.DrawLatex(0.2,0.8, "Low Gain Veto");
  ...
  ratio->Draw("histE");
  TString fname = "data_singlephoton_" + variable + "_2015_16_comparison_SBT";
  c1->SaveAs(fname+".png");
  c1->SaveAs(fname+".pdf");
  c1->SaveAs(fname+".eps");
  c1->SaveAs(fname+".C");
