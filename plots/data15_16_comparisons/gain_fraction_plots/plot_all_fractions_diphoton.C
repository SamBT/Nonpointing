...
void plot_all_fractions_diphoton(TString variable, TString xlabel, int rebin) {
  TFile *f1 = TFile::Open("/project/projectdirs/atlas/sambt/BatchOutput/nonpointing/data/combined_analyses/data15_HIGG1D1/output.root");
  TFile *f2 = TFile::Open("/project/projectdirs/atlas/sambt/BatchOutput/nonpointing/data/combined_analyses/data16_HIGG1D1/output.root");
  ...
  l.SetNDC();
  l.SetTextSize(0.04);
  l.DrawLatex(0.5,0.75,"Diphoton, p_{T} > 50, Barrel");
  ...
  leg.Draw();
  TString outfname = "data_diphoton_gain_fraction_vs_"+variable+"_15_16_comparison_SBT";
  c->SaveAs(outfname+".C");
