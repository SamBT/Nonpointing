void plot_diphoton_timing(int zbin, TString zdca_bin, int zlow, int zhigh) {
  ...
  h15->Scale(32.8846/3.2); //Scaling to 2016 lumi
  ...
  h16->Draw("histE");
  h15->Draw("histEsame");
  TString message = "Data, Diphoton, Projected Timing Distribution in SR";
  TLatex l;
  l.SetNDC();
  l.SetTextSize(0.04);
  ...
  ratio->Draw("histE");
  TString fname;
  fname.Form("data_projected_diphoton_SR_timing_z%i_%i_SBT",zlow,zhigh);
  c1->SaveAs(fname+".png");
  c1->SaveAs(fname+".pdf");
  c1->SaveAs(fname+".eps");
  c1->SaveAs(fname+".C");
