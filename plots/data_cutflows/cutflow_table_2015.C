void cutflow_table_2015(int begin, int end) {
  ofstream t1;
  TString t1_name = "cutflow_data15.tex";
  ...
  if (begin == 1) {
    ...
    t1 << "\\hline \n";
    t1 << " & & Total & Total/Lumi & Trigger/Lumi & $p_{T} > 150$ & Barrel & $E_{T}^{miss} < 20$ & $E_{T}^{miss} > 75$ & Total & Total/Lumi & Trigger/Lumi & $p_{T} > 50$ & Barrel & $E_{T}^{miss} < 20$ & $E_{T}^{miss} > 75$ \\\\ \n";
    t1 << "\\hline \n";
}
else if (end == 1) {
	t1 << "\\end{longtable} \n";
  ...
  while (!info.eof()) {
    ...
    t1 << std::fixed << std::setprecision(0) << run_num << "&" << std::fixed << std::setprecision(2) << lumi << "&" << std::setprecision(0) << hs->GetBinContent(1) << "&" << std::setprecision(2) << hs->GetBinContent(1)/lumi << "&" << hs->GetBinContent(2)/lumi << "&" << std::fixed << std::setprecision(0) << hs->GetBinContent(3) << "&" << hs->GetBinContent(4) << "&" << hs->GetBinContent(5) << "&" << hs->GetBinContent(6) << "&" << hd->GetBinContent(1) << "&" << std::fixed << std::setprecision(2) << hd->GetBinContent(1)/lumi << "&" << hd->GetBinContent(2)/lumi << "&" << std::fixed << std::setprecision(0) << hd->GetBinContent(3) << "&" << hd->GetBinContent(4) << "&" << hd->GetBinContent(5) << "&" << hd->GetBinContent(6) << " \\\\ \n";
    t1 << "\\hline \n";
  }
 }
}
